...
describe("eures-cli live smoke test", () => {
  test("search returns real results with populated id/title/url", async () => {
    const result = await runCLI(["search", "-q", "Softwareentwickler", "-l", "Berlin", "--limit", "5"]);
    const body = parseJSON<SearchResponse>(result);
    expect(body.results.length).toBeGreaterThan(0);
    for (const job of body.results) {
      expect(job.id).toBeTruthy();
      expect(job.title).toBeTruthy();
      expect(job.url).toMatch(/^https:\/\/europa\.eu\/eures\/portal\/jv-se\/jv-details\//);
    }
  });

  test("detail returns a readable description for a job found via search", async () => {
    const searchResult = await runCLI(["search", "-q", "Softwareentwickler", "-l", "Germany", "--limit", "1"]);
